...
コード変更を伴うタスクでは、**作業開始前**にリポジトリルートに `CODING_GUIDELINES.md` が存在するかを確認します。
- 存在する場合は**必ず読み込み**、その内容を遵守して実装します
- 規約はプロジェクト固有のレビューフィードバックを蓄積したもので、命名・構造・スタイル・アンチパターンなどのルールを含みます
- 実装中および完了前の自己検証時にも参照し、規約違反がないかを確認します
- ファイルが存在しない場合は、このステップをスキップしてプロジェクトの既存コードから慣習を読み取ります
