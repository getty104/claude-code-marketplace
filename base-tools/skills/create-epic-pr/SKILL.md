...
`$0` から Issue 番号を抽出する。以下のいずれかであることを確認:

- 数値のみ（例: `123`）
- `#`付き数値（例: `#123`）
- GitHub Issue URL（`.../issues/<番号>`）

該当しない場合や引数が空の場合は中断する。以降、抽出した番号を `<ISSUE>` と呼ぶ。
...
**完了条件**: `<ISSUE>` / Epic Issueタイトル / `<BASE>` が確定し、未コミット変更が無いこと。
...
対象ブランチ名は `cc-epic-<ISSUE>`。以下の優先順で処理する:

1. リモートに存在する場合（`git rev-parse --verify origin/cc-epic-<ISSUE>` が成功）:
   - `git checkout cc-epic-<ISSUE>` でチェックアウト（ローカルに無ければ自動で tracking branch が作られる）
   - `git pull --ff-only origin cc-epic-<ISSUE>` で最新化する。ff-only 失敗時は中断
2. ローカルのみに存在する場合（リモートに無く、`git rev-parse --verify cc-epic-<ISSUE>` が成功）:
   - `git checkout cc-epic-<ISSUE>` でチェックアウト
3. どちらにも存在しない場合は中断する。サブPRマージ前の早すぎる呼び出しを防ぐため、ブランチが無いケースでは自動作成せずに止める。

**完了条件**: `git rev-parse --abbrev-ref HEAD` が `cc-epic-<ISSUE>` を返すこと。
...
`<ISSUE>` 自身は除外し、重複も除去する。順序はコミットログ順を維持する。
...
```markdown
## 概要
Epic Issue #<ISSUE>「<Epic Issueタイトル>」に紐づくサブタスクをまとめた集約PRです。
...
`Closes #<ISSUE>` を必ず含めることで、Epic PR がマージされたとき Epic Issue も自動で close する。
...
heredoc は `<<'EOF'` でクォートしてシェル展開を抑止するため、本文中の `<ISSUE>` などのプレースホルダは heredoc に渡す前に実値へ置換しておくこと。
...
- 未コミット変更が存在する
- デフォルトブランチ名の取得失敗
- `cc-epic-<ISSUE>` ブランチがローカル / リモートのいずれにも存在しない
