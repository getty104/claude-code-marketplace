...
**責務の分担**: 本スキルは「既存Issue取得・タスク内容理解・コード再分析・元の依頼内容の保存と反映確認」までを担い、「本文整形・投稿前チェック・既存変更ログの verbatim 再掲・`## 依頼内容` の verbatim 再掲・`gh issue edit` 実行」は `post-issue-body` スキルへ委譲する。本文テンプレート・変更ログ追記ルール・投稿前チェックリスト・heredoc 投稿コマンドはすべて `post-issue-body` 側に集約されているため、本スキル内では再記述しない。

**「依頼内容」の保持**: 本スキルは Explore 再分析で `概要` `要件` `実装プラン` などを最新のコード状態に合わせて書き換える（title もリフレッシュの対象になり得る）。このとき、**元のIssueに人が書いた「原文の依頼」（＝実行前の title と description）が失われないように**、それを `## 依頼内容` セクションとして verbatim で残す。すでに `## 依頼内容` セクションが存在する Issue に対しては、そのセクションを verbatim で再掲して次のリフレッシュに引き継ぐ（`create-issue-from-issue-number` が繰り返し実行されても初回の依頼原文＝ title と body の両方が固定される）。
...
ステップ1で取得した title と body から、次のリフレッシュにも引き継ぐ「依頼内容」（原文の title＋description）を確定させる。この内容を **ステップ4で post-issue-body の `sections.依頼内容` にそのまま渡す**ことで、Explore 再分析結果に置き換わることなく description 内に残る。

判定ロジックは以下（機械的に判定し、ユーザーへの確認は不要）:

1. **既存bodyに `## 依頼内容` セクションが存在する場合**（過去に本スキルが実行済み、または他スキルが依頼内容を書き込んだIssue）
   ...
   - このセクションは前回の実行時に「タイトル」行と原文bodyの組で書き込まれているため、そのまま再掲すれば初回の title が失われない。今回のリフレッシュで title を書き換える場合でも `## 依頼内容` 内の元タイトルは触らない。
2. **既存bodyに `## 依頼内容` セクションが無い場合**（初回実行 or 旧フォーマットの Issue）
   - 「元のタイトル」と「元のbody」の両方を verbatim で結合したものを「依頼内容」とする。フォーマットは次のとおり固定（次回以降の抽出時に人が原文を判別しやすいようにするためと、`post-issue-body` の投稿前チェック（`##` テンプレート外の見出しを追加しない）に抵触させないため、`##` は使わず太字ラベルで表現する）:
     ...
     **元のタイトル**: <ステップ1で取得した title を verbatim>
     ...
     - body に他のセクション見出し（`## 概要` 等）が既に含まれていても構造には手を入れず、変更ログブロック以外はそのまま含める。Explore 再分析結果と原文の間で重複するセクションが発生し得るが、これは初回だけの現象で、以降のリフレッシュでは 1 のパスに落ちて安定する。可読性は損なうが、「原文を絶対に失わない」というユーザー要件を優先する。
3. **抽出結果が空文字列になった場合**（title が空文字で、かつ body も空 or 変更ログのみ、という極端なケース）— 依頼内容セクションを本文に含めないほうが自然なので、`sections.依頼内容` は渡さない（`post-issue-body` は該当セクションを本文から省略する）。title が空でなければ通常は該当しない。
...
2. 取得した body から `## 依頼内容` セクション本文を抽出する（見出し直後から次の `## ` 見出しまで、ステップ1.5と同じ切り出しルール）。
3. ステップ1.5で確定した「依頼内容」文字列と、抽出した本文を比較する。
   - **ステップ1.5で「依頼内容」が空だった場合** — 更新後bodyに `## 依頼内容` セクションが存在しないこと（存在すれば異常）。
   - **ステップ1.5で「依頼内容」が非空だった場合** — 更新後bodyから抽出した本文が、渡した文字列と**改行・空白まで含めて完全一致**すること。前後の空行1つ程度の差は許容してよいが、内容そのものの欠落・改変は不可。特に初回実行時は `**元のタイトル**: <原文タイトル>` の1行と、その後の空行、body本体、までがそのまま並んでいることを確認する（title 行の脱落は「原文が失われた」に相当する重大な逸脱なので必ず検出する）。
...
**完了条件**: 更新後Issueの `## 依頼内容` セクションが、ステップ1.5で確定した原文と一致していること（または双方が空であること）。
...
- `## 依頼内容` は「原文の verbatim 保持」が目的なので、要約したり体裁を整えたりしない。Explore 再分析の結果は必ず `概要` `要件` `実装プラン` 側に反映し、`依頼内容` は素の原文に留める
- `## 依頼内容` には**元の title** も `**元のタイトル**: <原文>` 形式で先頭に含める（初回実行時に組み立て、以降は verbatim 再掲）。Issue の title 自体を今回リフレッシュで書き換えたとしても、`## 依頼内容` 内の元タイトル行は絶対に書き換えない（そこは「人が最初に書いた依頼」の保存領域）
