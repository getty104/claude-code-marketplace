...
**「依頼内容」の保持**: 本スキルは Explore 再分析で `概要` `要件` `実装プラン` などを最新のコード状態に合わせて書き換える（title もリフレッシュの対象になり得る）。このとき、**元のIssueに人が書いた「原文の依頼」（＝実行前の title と description）が失われないように**、それを `<details><summary>依頼内容</summary>` の**折りたたみブロック**として verbatim で残す。折りたたみで描画するのは、Issue を開いた直後に真っ先に目に入るのが「Explore 再分析結果（概要・要件・実装プラン）」であるべきで、原文の依頼はデフォルト非表示・必要に応じて展開して読める形が実用的だから。すでに依頼内容ブロックが存在する Issue に対しては、その中身を verbatim で再掲して次のリフレッシュに引き継ぐ（`create-issue-from-issue-number` が繰り返し実行されても初回の依頼原文＝ title と body の両方が固定される）。旧フォーマット（裸の `## 依頼内容` 見出し）で残っている場合も、`post-issue-body` 側が読み込み時に折りたたみブロックへ詰め替えるため、本スキルは中身の抽出だけを担当する。

# Instructions

## 実行モードの制約: サブエージェント・サブスキル・Bashをバックグラウンド実行しないこと

本スキルは `claude-task-worker` の `create-issue` ワーカー（`cc-triage-scope` ラベル）から自動起動される想定である。ワーカーはスキルプロセスの同期完了を根拠にラベル遷移（例: `cc-issue-created` 付与）を進めるため、**本スキル内部で呼び出す `Agent` / `Skill` / `Bash` を絶対にバックグラウンド実行しないこと**。具体的には次を守る。

- `Agent` / `Skill` ツール呼び出し時に `run_in_background: true` を指定しない。特に Explore サブエージェントや `post-issue-body` の呼び出しは、返却された分析結果・投稿完了を受け取ってから次のフェーズに進む必要がある
...
**理由**: バックグラウンド化するとサブエージェント/子スキル/子プロセスの完了前に本スキルが終了してしまい、`claude-task-worker` は「スキルが正常完了した」と誤認して次のラベル遷移に進む。結果として、Issue の description 更新前にラベルが遷移して triage 系スキルが古い状態のまま起動される、`post-issue-body` の投稿完了前に別ワーカーが動き出す、といった状態壊れが発生する。ワーカー連携の同期性を担保するため、本スキルの内部処理はすべて同期実行で完結させる必要がある。
...
ステップ1で取得した title と body から、次のリフレッシュにも引き継ぐ「依頼内容」（原文の title＋description）を確定させる。この内容を **ステップ4で post-issue-body の `sections.依頼内容` にそのまま渡す**ことで、`post-issue-body` 側が本文の先頭に `<details><summary>依頼内容</summary>` の折りたたみブロックとして描画してくれる。Explore 再分析結果が本文の主体として真っ先に見え、原文はデフォルト折りたたみで残る形になる。

判定ロジックは以下（機械的に判定し、ユーザーへの確認は不要）:

1. **既存bodyに `<details><summary>依頼内容</summary>` ブロックが存在する場合**（前回本スキルを実行済み、新フォーマット）
   - `<summary>依頼内容</summary>` の閉じタグ直後から `</details>` の直前までを**中身だけ verbatim** で切り出し、それを「依頼内容」とする（`<details>` / `<summary>` / `</details>` タグ自体は含めない。`post-issue-body` 側でタグが再付与されるため）。
   - 先頭・末尾の空行1つは正規化してよいが、内部の空白行はトリムしない（原文の保持を優先）。
   - このブロックは前回の実行時に「タイトル」行と原文bodyの組で書き込まれているため、そのまま再掲すれば初回の title が失われない。今回のリフレッシュで title を書き換える場合でも、依頼内容ブロック内の元タイトルは触らない。
2. **既存bodyに旧フォーマットの `## 依頼内容` 見出しがある場合**（本フォーマット移行前に作られた Issue）
   - `## 依頼内容` の見出し直後から次の `## ` 見出しまたはEOFの直前までを**中身だけ verbatim** で切り出し、それを「依頼内容」とする。`post-issue-body` 側が受け取った時点で折りたたみブロックに詰め替えて書き出すため、本スキルからは中身を渡すだけでよい。
3. **どちらも存在しない場合**（初回実行 or 完全な旧フォーマットの Issue）
   - 「元のタイトル」と「元のbody」の両方を verbatim で結合したものを「依頼内容」とする。フォーマットは次のとおり固定（次回以降の抽出時に人が原文を判別しやすいようにするためと、`post-issue-body` の投稿前チェック（`##` テンプレート外の見出しを追加しない）に抵触させないため、`##` は使わず太字ラベルで表現する）:
     ...
     **元のタイトル**: <ステップ1で取得した title を verbatim>
     ...
     - title 行と body の間には必ず1行の空行を入れる（Markdown の段落境界を明示するため、および次回の抽出時に「タイトル行 vs body 冒頭」を混同しないため）。
     - body末尾の `<details><summary>変更ログ</summary> ... </details>` ブロックがある場合はそのブロックだけを除外する（変更ログは `post-issue-body` 側が別途 verbatim 再掲するため、二重掲載を防ぐ目的）。
     - body に他のセクション見出し（`## 概要` 等）が既に含まれていても構造には手を入れず、変更ログブロック以外はそのまま含める。Explore 再分析結果と原文の間で重複するセクションが発生し得るが、これは初回だけの現象で、以降のリフレッシュでは 1 のパスに落ちて安定する。しかも原文はデフォルト折りたたみで隠れるため、通常の閲覧では二重に見えない。
4. **抽出結果が空文字列になった場合**（title が空文字で、かつ body も空 or 変更ログのみ、という極端なケース）— 依頼内容ブロックを本文に含めないほうが自然なので、`sections.依頼内容` は渡さない（`post-issue-body` は該当ブロックを本文から省略する）。title が空でなければ通常は該当しない。
...
`sections.依頼内容` にはステップ1.5で抽出した文字列をそのまま渡す。改行やインデントを含む場合は YAML の `|` ブロックスカラーで表現し、勝手に整形・要約しないこと（原文の verbatim 保持が本セクションの目的）。
...
`post-issue-body` は本文全体を heredoc で上書きするため、想定外の整形ミスや YAML パース時の脱字によって、ステップ1.5で抽出した「依頼内容」が更新後のIssueに正しく載っていない可能性がある。次のリフレッシュに引き継ぐ原文が失われないよう、投稿完了直後に検証する。
...
3. ステップ1.5で確定した「依頼内容」文字列と、抽出した中身を比較する。
   ...
   - **ステップ1.5で「依頼内容」が非空だった場合** — 更新後bodyから抽出した中身が、渡した文字列と**改行・空白まで含めて完全一致**すること。前後の空行1つ程度の差は許容してよいが、内容そのものの欠落・改変は不可。特に初回実行時は `**元のタイトル**: <原文タイトル>` の1行と、その後の空行、body本体、までがそのまま並んでいることを確認する（title 行の脱落は「原文が失われた」に相当する重大な逸脱なので必ず検出する）。
   - **書き出しが折りたたみブロックになっていること** — 更新後bodyが旧フォーマットの裸の `## 依頼内容` 見出しのままだった場合も逸脱として扱う（`post-issue-body` は必ず折りたたみで書き出す規約なので、折りたたみになっていなければ整形失敗）。
...
- このスキルは**コードを一切変更しない**。Issue の更新・コメントは原則 `post-issue-body` 経由で行う（本スキル内で直接 `gh issue edit` を呼ぶのはステップ4.5の依頼内容反映が失敗したときのフォールバック用途に限る）
- 途中でユーザーに質問しない。確認したいことは `post-issue-body` へ「確認事項」として渡し、コメントとして残す
- 依頼内容は必ず `<details><summary>依頼内容</summary>` の**折りたたみ**で書き出す。裸の `## 依頼内容` 見出しは使わない（Explore 再分析結果を主体として先に見せ、原文はデフォルト非表示で残すのが本フォーマットの意図）
- 依頼内容ブロックの中身は「原文の verbatim 保持」が目的なので、要約したり体裁を整えたりしない。Explore 再分析の結果は必ず `概要` `要件` `実装プラン` 側に反映し、依頼内容は素の原文に留める
- 依頼内容ブロックには**元の title** も `**元のタイトル**: <原文>` 形式で先頭に含める（初回実行時に組み立て、以降は verbatim 再掲）。Issue の title 自体を今回リフレッシュで書き換えたとしても、依頼内容ブロック内の元タイトル行は絶対に書き換えない（そこは「人が最初に書いた依頼」の保存領域）
