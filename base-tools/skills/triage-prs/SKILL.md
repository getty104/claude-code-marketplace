...
`create-review-fix-plan`スキルを実行して修正プランを取得してください。

#### 3c. 修正プランに基づく判定とアクション

`create-review-fix-plan`の結果を分析し、以下の2パターンで判定し、**必ずどちらかのアクションを実行**してください。判定のみで終了せず、コマンドの実行まで確実に行ってください。
...
**必ず以下のコマンドを実行してマージしてください。判定だけで終了しないでください。**
