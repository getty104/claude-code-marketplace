...
- `## 依存関係` に書き込む Issue 番号は**呼び出し側で確定済みのもの**であることが前提。本スキルは渡された依存関係セクションをそのまま投稿する。先に作成したIssueの番号確定を待つ順序制御は呼び出し側の責務。
...
`triage-issue` や後続の Issue ライフサイクルスキルは、本文を読んでラベリング・タスク分解を行う。セクションの過不足、順序の入れ替え、見出し名のゆらぎは後続スキルの判断を狂わせ、人がレビューする際の可読性も損なう。どのスキルが作っても同じ構造になるよう、このフォーマットを揃えることが目的なので、独自のアレンジは加えない。
