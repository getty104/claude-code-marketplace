...
直近N日（デフォルト1日）のPRレビューコメントを収集し、繰り返し指摘されているかつ対応すべきと判断できる観点を、リポジトリルートの`CODING_GUIDELINES.md`に集約するスキルです。
このスキルが呼び出された際には、Instructionsに従って、レビューコメントの収集→クラスタリング→ガイドライン更新までを一貫して実行してください。
...
- 1日に1回以上の出現がない単発の指摘をルール化すること（ノイズ防止）
- スタイル好み・命名の主観・スコープ外提案など「対応不要寄り」の項目をルール化すること
- 既存ルールと意味重複する項目を別ルールとして追加すること（必ず統合・圧縮する）
...
- カレントディレクトリがgitリポジトリのルートであることを確認する（`git rev-parse --show-toplevel`の出力と一致するか）
- 一致しない場合は呼び出し元に「リポジトリルートで実行してください」と返して終了する
- 既存の`CODING_GUIDELINES.md`の有無を確認する。存在すれば全文を`Read`で読み込み、現在のルール構成・項目数・トーンを把握する

### 引数パース

`$ARGUMENTS`を空白区切りで最大2トークンに分解する。

- 1番目のトークン: 期間（日数）。省略時または非数値の場合は`1`を使用
- 2番目のトークン: 関連Issue番号（任意）。`#`プレフィックスは除去して数値部分のみ保持
...
Issue番号は後段のフェーズ5（PR作成）でcreate-prに渡すために使用する。指定なしの場合はPR本文の`Closes #N`行を省略する。

**完了条件**: リポジトリルートにいることが確認でき、既存`CODING_GUIDELINES.md`があれば現在の内容を把握済みで、日数と任意Issue番号が確定していること。
...
フェーズ0でパースした日数のみを第1引数として渡す（`$ARGUMENTS`をそのまま渡すと2番目のIssue番号トークンが余計な引数として渡ってしまうため、必ず日数だけを取り出して渡す）。引数を渡さない場合はスクリプト側の`DAYS="${1:-1}"`によりデフォルトの1日が使われる。
...
スクリプト側で`isMinimized: true`の会話コメントは既に除外済みだが、以下も**ルール化対象から外す**こと（クラスタリング対象には含めるが、抽出条件の判断時に除外する）:
...
収集したコメント群を**意味ベース**でクラスタリングする。字面が違っても以下のような同種の指摘は同じクラスタにまとめる:
...
- **使われなくなった指摘**: 直近6ヶ月のレビューに同種の指摘が一度も出現していない場合、本フェーズの分析対象外でもルール側で「廃止候補」として目印を付けず、**そのままにする**（削除判断は別スキルの責務）
...
`commit-push`はカレントブランチに対してコミット・pushを行うため、デフォルトブランチ上で実行されると本番ブランチに直コミットが入る。これを避けるため、PR作成用のfeature branchに必ず切り替える。
...
- `CURRENT_BRANCH = DEFAULT_BRANCH`の場合: 新しいfeature branchを作成して切り替える
  - ブランチ名は`chore/update-coding-guidelines-$(date +%Y%m%d-%H%M%S)`形式
...
`Skill`ツールで`commit-push` skillを呼び出す。引数なしで起動する（commit-push側がカレントブランチの差分とコミット履歴を自分で確認して戦略を選ぶ）。

### 5-2. create-pr skillの呼び出し

`Skill`ツールで`create-pr` skillを呼び出す。`create-pr`は`context: fork`でサブエージェントとして起動されるため、本スキルの会話文脈は引き継がれない。引数として渡せるのはIssue番号文字列のみ。
...
`create-pr`が`Closes #`（数字なし）行を残したまま本文を作る問題を、`gh pr edit`で修正する。
...
- **クラスタリングは意味ベース**: 同じキーワードが含まれていても文脈が違えば別クラスタ、逆に表現が違っても本質が同じなら同一クラスタとして扱う
- **採用基準は厳密に**: 1回しか出現していない指摘は重要そうでも採用しない（再発したときに改めて拾えば良い）。逆に「対応すべき類型」に含まれない繰り返し指摘も採用しない
- **既存ルールへの統合を優先**: ルール数の単調増加はドキュメントを死蔵させる。意味重複があれば必ず統合する
- **編集ツールは`CODING_GUIDELINES.md`のみ**: 他のソースコード・設定ファイル・別ドキュメントを編集しないこと
- **デフォルトブランチでの直コミット禁止**: `commit-push`を呼ぶ前にフェーズ4で必ずfeature branchへ切り替えること
- **差分なしならPR作成スキップ**: フェーズ3完了時点で`CODING_GUIDELINES.md`に差分がなければフェーズ4以降を実行しない（空コミット・空PRを作らない）
