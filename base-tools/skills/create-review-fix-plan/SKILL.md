...
### 2-1. レビューコメントの分析（コード調査は Explore に委譲）

まず各未解決スレッドの **意図** を読み取る（字面ではなく、レビュアーが懸念している根本問題）。

次に、指摘箇所の **現在のコード確認・修正対象の特定・影響範囲の見積もり** は `Explore` サブエージェントに委譲する。自前で `Read` を繰り返すより、ファイル横断の fan-out 探索を一度に行える Explore の方が「どこを直すべきか」を速く正確に特定できるためである。スレッドが複数ある場合は、同一メッセージ内で複数の Explore を **並列に** 起動して待ち時間を圧縮する。
