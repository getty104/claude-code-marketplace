...
本文は冒頭の「Issueフォーマット（厳守）」のテンプレートに従って組み立てる。各セクションをステップ1・2の分析結果で埋め、空になるセクションは「なし」で埋める。末尾の変更ログには「変更ログ（折りたたみ）の追記ルール」に従い、初版エントリを1行記載する（`date +%Y-%m-%d` で取得した日付 + 「初版作成 — <概要を一言>」）。
...
既存の Issue 構造を尊重しつつ、分析結果を反映したタイトルと本文に更新する。本文は冒頭の「Issueフォーマット（厳守）」のテンプレートに従い、`gh` 実行の直前に投稿前チェックを必ず実施する。末尾の変更ログは「変更ログ（折りたたみ）の追記ルール」に従い、ステップ1で取得した既存本文のエントリを verbatim で再掲したうえで、今回の更新分を1行追記する（旧フォーマットでブロックが無ければ新規作成する）。
